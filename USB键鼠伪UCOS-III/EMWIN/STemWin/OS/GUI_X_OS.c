...
int GUI_X_WaitKey(void)
{
	...
	if(KeyPressed == 0)
	{
		OSSemPend(KeySem,0,OS_OPT_PEND_BLOCKING,0,&err);
	}
	r = KeyPressed;
	KeyPressed = 0;
	return (r);
...
{
	OS_ERR err;
	OSSemPost(DispSem,OS_OPT_POST_1,&err);
}

void GUI_X_Lock(void)
{
	OS_ERR err;
	OSSemPend(DispSem,0,OS_OPT_PEND_BLOCKING,0,&err);
}
...
{
	OS_ERR err;
	OSSemPost(EventSem,OS_OPT_POST_1,&err);
}
