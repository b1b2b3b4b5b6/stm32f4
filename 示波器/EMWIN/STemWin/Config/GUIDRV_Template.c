...
{
	ili9325_draw_point565(x,y,PixelIndex);
}
...
static unsigned int _GetPixelIndex(GUI_DEVICE * pDevice, int x, int y) {
    ...
    GUI_USE_PARA(y);
    {
		ili9325_read_point565(x,y,&PixelIndex);
    }
    #if (LCD_MIRROR_X == 0) && (LCD_MIRROR_Y == 0) && (LCD_SWAP_XY == 0)
      #undef xPhys
      #undef yPhys
    #endif
	return (unsigned int)PixelIndex;
}
...
{
	ili9325_draw_window565(x0,y0,x1,y1,LCD_COLORINDEX);
}
...
{
	ili9325_check_direction((u16 *)(&x),(u16 *)(&y));
	ili9325_set_point(x,y);
	for (;xsize > 0; xsize--, p++)
	{
		LCD->data=*p;
    }
