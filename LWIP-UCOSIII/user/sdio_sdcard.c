...
SD_Error SD_PowerON(void)
{
	...
	u32 SDType=SD_STD_CAPACITY;
	 ...
	 /*初始化时的时钟不能大于400KHz*/ 
	SDIO_InitStructure.SDIO_ClockDiv = SDIO_INIT_CLK_DIV;	/* PLLCLK =48MHz, SDIOCLK = 48MHz, SDIO_CK = HCLK/(178 + 2) = 400 KHz */
  ...
  SDIO->DCTRL=0x0;	//数据控制寄存器清零(关DMA) 
	...
	}else if(DeviceMode==SD_DMA_MODE)
	{
		...
		SDIO->MASK|=(1<<1)|(1<<3)|(1<<8)|(1<<5)|(1<<9);	//配置需要的中断 
 		...
 		while(((DMA2->LISR&(1<<27))==RESET)&&(TransferEnd==0)&&(TransferError==SD_OK&&(timeout!=0))) timeout--;//等待传输完成 
