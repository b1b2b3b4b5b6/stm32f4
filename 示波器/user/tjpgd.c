...
u8 jpeg_mallocall(void)
{
	...
	if(f_jpeg==NULL)return 1;			//申请内存失败.
	jpeg_dev=(JDEC*)mymalloc(sizeof(JDEC));
	if(jpeg_dev==NULL)return 1;		//申请内存失败.
	jpg_buffer=(u8*)mymalloc(JPEG_WBUF_SIZE);
	if(jpg_buffer==NULL)return 1;		//申请内存失败.
	return 0;
}
//释放内存
...
u32 jpeg_out_func_fill(JDEC* jd,void* rgbbuf,JRECT* rect)
{
	...
	ili9325_fill_window565(rect->left+x_start-1,rect->top+y_start-1,width,height,rgbbuf);
...
//返回值:0,解码成功;其他,解码失败.
u8 jpg_decode(const u8 *filename,u16 x_off,u16 y_off)
{
	u8 res=0;	//返回值
	x_start=x_off;
	y_start=y_off;
	...
	if(res==0)
	{
		...
		if(res==FR_OK)//打开文件成功
		{
			...
			if(res==FR_OK)	res=jd_decomp(jpeg_dev,jpeg_out_func_fill,0);
