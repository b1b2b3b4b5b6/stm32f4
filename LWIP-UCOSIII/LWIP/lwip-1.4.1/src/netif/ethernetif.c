...
static err_t low_level_init(struct netif *netif)
{
	...
	netif->flags = NETIF_FLAG_BROADCAST|NETIF_FLAG_ETHARP|NETIF_FLAG_LINK_UP;//允许该网卡广播和ARP功能		
	return ERR_OK;
} 
...
static err_t low_level_output(struct netif *netif,struct pbuf *p)
{
	...
	int l=0;
	u8 *buffer;
	buffer=mymalloc(p->tot_len);	//申请内存
...
{  
	...
	p=pbuf_alloc(PBUF_RAW,len,PBUF_POOL);	//pbufs内存池分配pbuf,安全
	if(p!=NULL)
	{
		for(q=p;q!=NULL;q=q->next)
		{
